...
One of the crux decisions was when and where to calculate the standard deviations and volatility rankings. There are two main options regarding when:
...
Given the time constraints, I didn't consider using tools or languages I'm not currently very familiar with. That said, I actually think NodeJS is a good choice here. Most of the heavy lifting is being done in the database so the application code is really just orchestration and request handling, which Node is very good at. But I do apologize for not following the instruction to use Java or Python.
...
- API Key is stored in plain text in the repo and loaded via an environment variable. Secrets like this should be handled in way that doesn't expose the value to anything other than the service itself at runtime.
...
I definitely spent more than 6 hours on this. Including this Readme and everything else it was probably more like 8-10 hours. But I didn't use any AI assistance (apart from basic code-completion in WebStorm). Some things that contributed to the additional time are:
